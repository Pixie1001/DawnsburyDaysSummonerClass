        ...
        public static Dictionary<SummonerSpellId, SpellId> LoadSpells() {
            ...
            spellList.Add(SummonerSpellId.EidolonsWrath, ModManager.RegisterNewSpell("EidolonsWrath", 3, (spellId, spellcaster, spellLevel, inCombat, spellInformation) => {
                return Spells.CreateModern(IllustrationName.DivineWrath, "Eidolon's Wrath", new[] { tSummoner, Trait.Focus, Trait.Evocation, Trait.Uncommon },
                        ...
                        (spellcaster != null && spellcaster.HasEffect(qfEidolonsWrath) ? HumanizeDamageKind((DamageKind)spellcaster.QEffects.FirstOrDefault(qf => qf.Id == qfEidolonsWrath).Tag) + " damage" : "damage of the type chosen when you took the Eidolon's Wrath feat") + ".",
                ...
                .WithSoundEffect(SfxName.Fireball)
                    .WithEffectOnEachTarget((Delegates.EffectOnEachTarget)(async (spell, caster, target, _) => {
                        ...
                        CheckResult result = CommonSpellEffects.RollSavingThrow(target, spell, Defense.Fortitude, caster => GetSummoner(caster).Spellcasting.PrimarySpellcastingSource.GetSpellSaveDC());
                        DamageKind dk = (DamageKind)caster.QEffects.FirstOrDefault(qf => qf.Id == qfEidolonsWrath).Tag;
