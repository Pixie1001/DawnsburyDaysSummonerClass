...
namespace Dawnsbury.Mods.Classes.Summoner {
    ...
    "from casting the angelic messenger ritual, even if they somehow learn it.";
        ...
        internal static IEnumerable<Feat> LoadSubclasses() {
            ...
            yield return CreateEidolonFeat(scDraconicEidolonCunning, "Your eidolon is a cunning wyrm.", null, "{b}Breath Weapon{/b} {{icon:TwoActions} Your eidolon exhales a line or cone of energy and deal 2d6 of the damage associated with your eidolon's dragon type to each target. You can't use breath weapon again for 1d4 rounds.\n", new int[6] { 1, 4, 1, 2, 1, 1 }, 1, 4);
            yield return CreateEidolonFeat(scDraconicEidolonMarauding, "Your eidolon is a fierce marauding drake.", null, "{b}Breath Weapon{/b} {{icon:TwoActions} Your eidolon exhales a line or cone of energy and deal 2d6 of the damage associated with your eidolon's dragon type to each target. You can't use breath weapon again for 1d4 rounds.\n", new int[6] { 4, 2, 3, 0, 0, 0 }, 2, 3);
            ...
            yield return CreateEidolonFeat(scElementalEidolonPrimordial, "Your eidolon is a hulking undead abomination.", (subclasses.Last() as EidolonBond).AbilityText, (subclasses.Last() as EidolonBond).ActionText, new int[6] { 4, 2, 3, -1, 1, 0 }, 2, 3);
            yield return CreateEidolonFeat(scElementalEidolonAdaptable, "Your eidolon is a ghoulish stalker of the living.", (subclasses.Last() as EidolonBond).AbilityText, (subclasses.Last() as EidolonBond).ActionText, new int[6] { 1, 4, 3, 0, 1, 0 }, 1, 4);
            ...
            subclasses.Add(new EidolonBond(scFeyEidolon, FeyEidolonFlavour, FeyEidolonCrunch, Trait.Primal, new List<FeatName>() { FeatName.Nature, FeatName.Deception },
                new Func<Feat, bool>(ft => ft.HasTrait(tAlignment)), new List<Trait> { })
            .WithOnSheet((Action<CalculatedCharacterSheetValues>)(values => {
            ...
            subclasses.Add(new EidolonBond(scPsychopompEidolon, PsychopompEidolonFlavour, PsychopompEidolonCrunch, Trait.Divine, new List<FeatName>() { FeatName.Religion, FeatName.Intimidation },
                ft => new FeatName[] { ftANeutral }.Contains(ft.FeatName), new List<Trait> { })
        ...
        private static void UndeadEidolonLogic(Creature eidolon, Creature summoner) {
            eidolon.AddQEffect(new QEffect("Negative Essence", "Your eidolon is undead, though unlike true undead, your connection grants it a sliver of life. It has negative healing, but instead of the usual immunities it gets a +2 circumstance bonus to death, disease and poison effects. Additionally, it gains a +5 bonus to staunch persistent bleed damage.") {
                ...
                BonusToDefenses = (self, action, defence) => {
                    if (defence != Defense.AC && action != null && action.Traits.ContainsOneOf(new Trait[] { Trait.Poison })) {
            ...
            if (eidolon.Level >= 7) {
                eidolon.AddQEffect(new QEffect() {
                    ProvideStrikeModifierAsPossibility = item => {
                        ...
                        CombatAction drainLife = new CombatAction(eidolon, IllustrationName.SuddenBlight, "Drain Life", new Trait[] { Trait.Divine, tEidolon, Trait.Necromancy, Trait.Negative }, "", Target.Ranged(1));

                        StrikeModifiers modifiers = new StrikeModifiers() {
                            OnEachTarget = async (a, d, strikeResult) => {
                                CheckResult saveResult = CommonSpellEffects.RollSavingThrow(d, action, Defense.Fortitude, _ => summoner.Spellcasting.PrimarySpellcastingSource.GetSpellSaveDC());
                        ...
                        action.Illustration = new SideBySideIllustration(IllustrationName.SuddenBlight, item.Illustration);
                        return (ActionPossibility)action;
                    }
                });
            }
                ...
                eidolon.AddQEffect(new QEffect() {
                    ProvideMainAction = self => {
                        return (ActionPossibility)new CombatAction(eidolon, illSoulWrench, "Soul Wrench", new Trait[] { Trait.Necromancy, Trait.Magical, Trait.Incapacitation },
                            ...
                            .WithAdditionalConditionOnTargetCreature((a, d) => new Trait[] {
                                Trait.Incorporeal, Trait.Celestial, Trait.Fiend, Trait.Archon, Trait.Elemental, Trait.Construct }
                        ...
                        .WithSavingThrow(new SavingThrow(Defense.Will, _ => summoner.Spellcasting.PrimarySpellcastingSource.GetSpellSaveDC()))
                    ...
                    ProvideContextualAction = self => {
                        ...
                        .WithEffectOnSelf(async (action, user) => {
                            ...
                            CheckResult result = CommonSpellEffects.RollSavingThrow(target, action, Defense.Will, _ => summoner.Spellcasting.PrimarySpellcastingSource.GetSpellSaveDC());
            ...
            if (eidolon.Level >= 7) {
                eidolon.AddQEffect(new QEffect() {
                    ProvideMainAction = self => {
                        ...
                        .WithSavingThrow(new SavingThrow(Defense.Reflex, _ => summoner.ClassOrSpellDC()))
