    ...
    public static class SummonerClassLoader {
        ...
        private static IEnumerable<Feat> CreateFeats() {
            ...
            yield return new EvolutionFeat(ModManager.RegisterFeatName("Eidolon's Opportunity {icon:Reaction}"), 6,
                ...
                new Trait[] { tSummoner, tTandem }, e => e.AddQEffect(QEffect.AttackOfOpportunity()), null);
                            ...
                            self.AddQEffect(new QEffect("Ostentatious Arrival Toggled", "Your next summoning spell will create a 10-foot burst, dealing 1d4 fire damage to all creatures caught inside.") {
                                ...
                                YouBeginAction = async (qf, action) => {
                                    ...
                                    if (target != null) {
                                        ...
                                        await CommonAnimations.CreateConeAnimation(qf.Owner.Battle, target.ToCenterVector(), DetermineTilesCopy(qf.Owner, burst, target.ToCenterVector()).TargetedTiles.ToList(), 20, ProjectileKind.Cone, IllustrationName.FlamingSphere);
                                        // Target
        ...
        private static Feat CreateEidolonFeat(FeatName featName, string flavorText, int[] abilityScores, int ac, int dexCap) {
            ...
            .AddQEffect(new QEffect("Eidolon", "This character can summon and command an Eidolon.") {
                StartOfCombat = (Func<QEffect, Task>)(async qfSummonerTechnical => {
                    ...
                    if (handwraps != null) {
                        ...
                        foreach (ItemModification rune in weapons[0].ItemModifications) {
                            eidolonHandwraps.WithModification(rune);
                        }
                        eidolonHandwraps.Name = weapons[0].Name;
                        ...
                        foreach (ItemModification rune in weapons[1].ItemModifications) {
                            eidolonHandwraps.WithModification(rune);
                        }
                        eidolonHandwraps.Name = weapons[1].Name;
                ...
                StateCheckWithVisibleChanges = (async qf => {
                    ...
                    QEffect? drained = qf.Owner.QEffects.FirstOrDefault(effect => effect.Id == QEffectId.Drained);
                    if (drained != null) {
            ...
            .AddQEffect(new QEffect() {
                ProvideActionIntoPossibilitySection = (qf, section) => {
                    ...
                    if (qf.Owner.QEffects.FirstOrDefault(effect => effect.Id == qfInvestedWeapon) != null) {
                        itemOptions.Remove((Item)qf.Owner.QEffects.FirstOrDefault(effect => effect.Id == qfInvestedWeapon).Tag);
                    }
                    ...
                    foreach (Item item in itemOptions) {
                        ...
                        .WithEffectOnSelf(async self => {
                            ...
                            QEffect? oldInvestedEffect = qf.Owner.QEffects.FirstOrDefault(effect => effect.Id == qfInvestedWeapon);
                            ...
                            foreach (ItemModification rune in item.ItemModifications) {
                                eidolonHandwraps.WithModification(rune);
                            }
                            eidolonHandwraps.Name = item.Name;
            ...
            return new Creature(illustration1, name1, (IList<Trait>)traits, level, perception, speed1, defenses, hp, abilities2, skills)
                ...
                "Your eidolon gains all of your skill proficiancies and uses your spell attack and save DC for its special abilities.") {
                    ...
                    StateCheckWithVisibleChanges = (async qf => {
                        ...
                        // Handle handwraps
                        if (summoner.HasEffect(qfInvestedWeapon)) {
                            QEffect investedWeaponQf = summoner.QEffects.FirstOrDefault(effect => effect.Id == qfInvestedWeapon);
