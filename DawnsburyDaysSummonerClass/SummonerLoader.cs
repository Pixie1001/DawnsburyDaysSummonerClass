    ...
    public static class SummonerClassLoader {
                            ...
                            self.AddQEffect(new QEffect("Ostentatious Arrival Toggled", "Your next summoning spell will create a 10-foot burst, dealing 1d4 fire damage to all creatures caught inside.") {
                                ...
                                YouBeginAction = async (qf, action) => {
                                    ...
                                    if (target != null) {
                                        ...
                                        await CommonAnimations.CreateConeAnimation(qf.Owner.Battle, target.ToCenterVector(), DetermineTilesCopy(qf.Owner, burst, target.ToCenterVector()).TargetedTiles.ToList(), 20, ProjectileKind.Cone, IllustrationName.FireRay);
                                        // Target
        ...
        private static Creature CreateEidolon(FeatName featName, int[] abilityScores, int ac, int dexCap, Creature summoner) {
            ...
            List<KeyValuePair<Trait, Proficiency>> skillProfs = summoner.Proficiencies.AllProficiencies.ToList()
                ...
                .Where(t => UtilityFuncs.Trait2Skill(t.Key) != null).ToList();
        ...
        private static Creature CreateEidolonBase(string name, Creature summoner, int[] abilityScores, int ac, int dexCap) {
            ...
            int hp = summoner.MaxHP + summoner.DrainedMaxHPDecrease;
            ...
            foreach (Trait trait in skillTraits) {
                ...
                Skill? skill = (Skill)UtilityFuncs.Trait2Skill(trait);
        ...
        private static void HealthShareSafetyCheck(Creature self, Creature partner) {
            if (self.MaxHP + self.DrainedMaxHPDecrease < partner.MaxHP + partner.DrainedMaxHPDecrease) {
                self.Heal(DiceFormula.FromText($"{partner.MaxHP + partner.DrainedMaxHPDecrease - self.MaxHP + self.DrainedMaxHPDecrease}", "Eidolon Health Share (failsafe)"), null);
            } else if (self.MaxHP + self.DrainedMaxHPDecrease > partner.MaxHP + partner.DrainedMaxHPDecrease) {
                partner.Heal(DiceFormula.FromText($"{self.MaxHP + self.DrainedMaxHPDecrease - partner.MaxHP + partner.DrainedMaxHPDecrease}", "Eidolon Health Share (failsafe))"), null);
            }
        }
